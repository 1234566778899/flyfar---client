...
export const GameTestApp = () => {
    const navigate = useNavigate();
    const [optionActive, setOptionActive] = useState('problem');
    const { taskId } = useParams();
    const [isLoading, setIsLoading] = useState(false);
    const [task, setTask] = useState(null);
    const { owner, challenge, setChallenge } = useContext(MainContext);
    ...
    const sendResult = () => {
        if (!isLoading) {
            ...
            axios.post(`${CONFIG.uri}/results/add`, { result: code, title: task.title, lenguaje: owner.favoriteLenguaje, user: owner._id, task, challenge: challenge._id, time })
                .then(res => {
                    const { score } = res.data;
                    const tasks = [...challenge.tasks].map(x => x._id == taskId ? ({ ...x, score }) : x);
                    setChallenge({ ...challenge, tasks });
                    navigate(`/admin/test/${challenge._id}`);
                })
                .catch(error => {
                    setIsLoading(false);
                    showInfoToast('Error');
        ...
        <div  >
            <div className="container inter">
                ...
                <div className='mb-3' style={{ display: 'flex', color: '#717171', fontWeight: 'bold', alignItems: 'center' }}>
                    <span style={{ cursor: 'pointer' }} onClick={() => navigate(`/admin/test/${challenge._id}`)} className='me-2'>Todos los desafios</span>
